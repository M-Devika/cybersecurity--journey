...
* **DHCP Request** — The client requests or confirms the offered configuration.
...
The analysis therefore distinguishes between the expected process and the traffic actually observed.
...
More specific filters can also be used when investigating particular DHCP message types.
...
This practical reinforced an important network-analysis principle:

> **Analyze the evidence that is present in the packet capture rather than assuming that the expected traffic was captured.**
